...
Domain-specific node and edge seeds are organized under `src/v2/knowledge/seeds`. Ordinary genuine relations that span current Domain assignments live in a shared seed; they have no special cross-domain business status. `pnpm audit:knowledge` prints the current quality report and performs relation validation.
