...
Goal text is a planning request, not an independent Goal lifecycle entity in V1B. The LLM language adapter proposes a bounded (at most six) minimal target set with one primary outcome and a direct-outcome reason for every identity. An independent structured semantic check rejects real-but-incoherent IDs, while the product service revalidates every ID against visible active Knowledge. Continue Search retains the prior Goal and validated target identities and stores preference/constraint refinement separately; a new Goal action creates an independent outcome result. Clarification continuation requires an explicit owning message identity. Ambiguous and unsupported Goals terminate with clarification rather than scope expansion. Prerequisite closure uses only factual `prerequisite` edges and is deterministic, deduplicated, and cycle-reporting.
