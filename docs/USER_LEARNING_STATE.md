...
## Criterion State and Recommendation Foundation

`explore -> learning -> learned -> practicing -> mastered` remains the monotonic **progression stage**, with the existing mastery policy. It is not the fine-grained learner state. Existing `KnowledgeNode.masteryCriteria` and revision `masteryCriteria` are authored prose standards. The normalized `mastery_criteria` records operationalize assessable aspects of those standards with stable identity, explicit version and Knowledge revision provenance; prose is never parsed to manufacture identity, outcomes or state. No historical completion is backfilled as performance.

`LearnerCriterionState` is a deterministic read projection keyed by learner × criterion version, using `criterion-state-v1`. The same source-independent estimator accepts `CriterionEvidence` from source adapters; Micro attempts are the only activated performance adapter in this release. Instruction/exploration observations and client-reported H5P completion do not enter that estimator. Source facts remain in the immutable attempt table. There is no separately writable learner-criterion-state table.

The estimator deduplicates evidence IDs and orders the facts by database sequence. Its judgment uses at most the latest five observations. No observations means attainment/stability `unknown` and confidence `low`. Latest incorrect means `insufficient`; latest correct means `developing` until three consecutive correct observations support `achieved`. Fewer than two observations leaves stability `unknown`; three consecutive correct observations yield `stable`; mixed observations ending correct yield `improving`; other repeated performance is `unstable`. Evidence support is low below three observations, medium at three/four, and high at five or more. Confidence is support for the judgment, not psychological confidence, a probability, or a mastery score. Independence, transfer and retention remain `unknown`; missing evidence is not failure. These conservative finite rules are an engineering baseline, not a validated psychometric scale. They never advance progression/mastery.

Every state carries estimator/criterion version, the bounded explanatory evidence IDs, count and through-sequence. A decision records the compact states actually used, their hash, and its evidence cutoff. Reconstruct its state by applying its estimator version to its criterion versions and observations up to that cutoff, not by substituting today's active criterion definitions. A later attempt's sequence provides the successor cutoff. User-scoped sequence allocation is serialized in the same transaction as attempt/completion, preventing late commits below a captured cutoff.

Recommendation is separate from route projection. `generateCandidates` consumes authenticated server-loaded route, targets, real published Micro assets, progression and completion facts, plus factual Course-local prerequisites. It excludes completed resources and blocked nodes and retains a blocked/contentless route frontier as a stop. The candidate contract also represents existing Course/Assignment/Material identities, but the learning recommendation does not promote the independent Assignment backlog or supporting Material into its primary action. Course visibility/ownership is checked before generation; viewing an available Course does not activate membership. The first authorized Course learning action activates membership, preserving progressive joining and reactivation semantics.

All policies share `LearningAction`, `CandidateAction`, and `RecommendationSelection`. `FixedPolicy` follows the existing `course-rule-v4` frontier without fine-state reordering. `RulePolicy` prioritizes an already-legal, unfinished Micro explicitly mapped to an insufficient criterion; otherwise it delegates to Fixed. Unknown state is never treated as achieved or failed. A policy cannot supply an action outside the candidate set. Historical progression that conflicts with hard prerequisites produces a Course-level blocked reason instead of reintroducing a filtered Micro. `ModelPolicy` is an unsupported extension contract, with no model, scores or artificial weights.

`course_recommendation_policies` is server configuration outside authoring Course definitions, so Publish cannot reset it. An authenticated administrator can POST `{ "policyKey": "fixed" | "rule_v1" }` to `/api/navigation?courseId=...`; the server verifies the profile role. Missing configuration means Fixed. Query parameters, browser metadata, and learner requests cannot select the policy. The response retains the route's `policyVersion` and exposes separate recommendation key/version fields. Both policy versions participate in the input hash. The sole decision authority remains `navigation_decisions`, with candidates, selected action, reason, state summary/hash, estimator version and evidence cutoff. Unchanged inputs reuse the decision.

The Course CTA and Micro continuation carry the clicked decision ID and exact Micro path identity through existing navigation state. On submission the server revalidates the decision's owner, Course, Knowledge and selected resource. Manual entries have no recommendation attribution; the server never guesses the last recommendation was clicked. `GET /api/micro?view=learning-data` is authenticated and returns own facts, definitions and current states; `throughSequence` replays the observation cutoff. Add `decisionId` for historical version-specific replay and hash verification; optionally add `throughSequence` for its successor state. Saved criterion references and own facts remain replayable when current Knowledge definitions are archived or invisible; unavailable descriptions are omitted. No anonymous learner state/session is invented.

Authoritative progression, Micro progress, completion Evidence and accepted Assignment evaluation are server-written. Authenticated clients have no write grants to the first three and cannot insert or modify accepted Assignment rows. Bulk progress synchronization preserves canonical acceptance instead of trusting the browser. Non-evaluative Assignment progress remains compatible with the existing API. The frontend legacy `completed` alias was already rejected by the database status CHECK; it is not accepted performance and cannot unlock hard dependencies. This release does not add a Workflow evaluator or migrate those legacy reports.

Course Navigation scopes Criterion definitions and attempt/cutoff SQL to the Course Knowledge identities, chunked to bounded PostgREST URLs. It does not filter by source Course because shared Knowledge observations remain reusable. Historical Decision replay continues to use saved identities/versions and own historical facts independently of today’s active catalog. Learner-facing recommendation reasons must originate from the actual Decision reasonCode/server explanation; only admin sees policy metadata.
