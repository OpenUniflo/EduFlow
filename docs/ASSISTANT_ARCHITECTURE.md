...
Goal planning keeps its durable write boundary outside the LLM tool loop. Explicit Goal mode first uses the server-side structured language adapter to interpret novice language against the visible active Knowledge catalog. The adapter proposes one primary outcome, at most six direct target identities, and a reason for each; an initial unsupported verdict receives one independent catalog-grounded audit so a transient semantic miss cannot silently hide real visible Knowledge, and a second structured semantic check rejects incoherent real IDs. Both passes remain language interpretation only: the product Goal Planning Service revalidates identity/visibility and remains the authority for factual prerequisite closure and deterministic Course coverage plus scope-overhead metrics. Continue Search is an explicit refinement action and therefore preserves the prior Goal text and validated Target set while storing the refinement separately. Replacing the outcome uses a new Goal action and creates an independent planning result. A reply is treated as clarification continuation only when it carries the owning clarification message identity; unrelated session history does not imply continuation.
