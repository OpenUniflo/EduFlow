...
Goal planning keeps its durable write boundary outside the LLM tool loop. Explicit Goal mode first uses the server-side structured language adapter to interpret novice language against the visible active Knowledge catalog. The adapter proposes one primary outcome, at most six direct target identities, and a reason for each; a second structured semantic check rejects incoherent real IDs. The product Goal Planning Service still revalidates identity/visibility and remains the authority for factual prerequisite closure and deterministic Course coverage plus scope-overhead metrics. Preference-only refinement preserves the prior validated Target set unless the learner uses explicit outcome-change language.

The same authenticated `/api/assistant` handler accepts `plan-goal`, `refine-goal`, `use-existing-course`, `prepare-course-brief`, and `course-creator-proposal` actions. Course Search and Course Creation Brief results are versioned structured content on ordinary Assistant messages. Each action references the owning persisted planning/Brief message, and the server reloads its snapshot and revalidates current Knowledge/Course visibility. Match levels explain and rank; they do not decide which actions are available. Continue Search appends a new result. Preparing a Brief performs no Course write. Course Creator intent routing is deliberately small: obvious Navigate commands are deterministic and bypass the mutation provider, Explain yields no operations or Apply action, and Edit alone may produce a bounded product-owned operation list. Edit remains Proposal -> Preview/Diff -> deterministic validation -> explicit confirmation -> Apply. Provider unavailability is reported separately from invalid proposal output. Personal Course persistence and completion stay in the Course API, never the model tool loop.
