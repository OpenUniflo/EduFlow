...
Course, Material, Knowledge, learner context, and a supported Golden scenario are Creation References, not creation modes. Steps 1-4 are a client design preview anchored by the persisted Brief; changing an upstream confirmed output invalidates downstream results. Raw `requestedAdjustments` remains a supplemental requirement until a stage-owned structured proposal separates foundation, constraint, preference, and scope changes; punctuation is not a semantic parser. Scope Apply always normalizes factual prerequisite closure from the current Target set; prerequisite labels are not directly editable. Step 4 distinguishes actual included coverage, reference assets available to reuse, missing coverage, and a desired Asset Plan. Desired markers never manufacture Material, Micro, or Assignment. Step 5 is the first Course-domain write and displays the real persisted result before another learner confirmation. `courses.creation_brief_message_id` is a unique recovery/idempotency key, so refresh, re-open, save, and concurrent retries preserve one Course identity. `courses.creator_metadata` contains only the confirmed learner foundation, time constraint, preferences, raw adjustment, and Desired Asset Plan required to restore the Creator; Course/Knowledge/curriculum authority remains in normalized tables. Draft Preview and My Courses expose owner-only Continue Creation/Preview actions using that persisted Brief link. Step 6 reuses the real Course graph/learner projection and changes `draft -> published` only after an explicit user action and server structural validation. Missing assets remain warnings. Arbitrary Material extraction/alignment and full asset generation remain deferred.
...
The learner six-stage Creator and teacher/admin Design Mode are currently separate UI entry points over the same Course tables, repository, Course graph projection, and learner preview routes. Learner Draft Preview returns by `creation_brief_message_id`; Design Mode returns to Teaching Management. Publish remains an explicit product action in both presentations, but their client-side proposal/validation orchestration is not yet one literal function. This is an intentional MVP boundary, not a second Course ontology.
