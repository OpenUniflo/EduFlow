...
Course, Material, Knowledge, learner context, and a supported Golden scenario are Creation References, not creation modes. Steps 1-4 are a client design preview anchored by the persisted Brief; changing an upstream confirmed output invalidates downstream results. Scope Apply always normalizes factual prerequisite closure from the current Target set; prerequisite labels are not directly editable. Step 4 distinguishes actual included coverage, reference assets available to reuse, missing coverage, and a desired Asset Plan. Desired markers never manufacture Material, Micro, or Assignment. Step 5 is the first Course-domain write and displays the real persisted result before another learner confirmation. `courses.creation_brief_message_id` is a unique recovery/idempotency key, so refresh, re-open, save, and concurrent retries preserve one Course identity. Step 6 reuses the real Course graph/learner projection and changes `draft -> published` only after an explicit user action and server structural validation. Missing assets remain warnings. Arbitrary Material extraction/alignment and full asset generation remain deferred.
