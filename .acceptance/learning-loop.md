...
## Micro V2 experience repair — 2026-09-06 (single Agent)

Baseline: branch `feat/gold-course-learning-loop`, HEAD `02d6d6329d3fe2eaa0ce0b81968cab4ff42eb958`, clean worktree. Typecheck/lint PASS; 79 files / 493 tests PASS; build PASS in 8.46s with pre-existing large bundle/H5P CSS warnings. Existing runtime: database-backed text body + shared Flow/Simulation/Data Transformation; `reviewCursor` previously only entered after full completion; pinned success retains the completed step until Continue. No math dependency existed. Existing React Flow, Motion and dnd-kit are reused.

Scope: RT01, CTX01, CDS525-K012, CDS525-K021 only; CDS525-K001 remains the H5P compatibility gate. No new course, Knowledge identity, runtime type, LLM provider, Function or progress table. Content update is data-only, not Hosted schema DDL.

Findings/fixes: plain string formulas → KaTeX with explicit delimiters and safe text fallback; static mechanism Explain → existing runtime with demonstration presentation; completed-only review → previous/next/current review navigation while retaining and pausing formal interaction; runtime teaching strings → factual reasons plus authored teaching feedback/explanation. Student precheck caught malformed JSX TeX escaping and an initial Motion SVG coordinate error; both corrected before acceptance. Existing source-string tests were adjusted for the relocated Step panel and reason contract without removing their checks.

Automated interim: 80 files / 499 tests PASS; build PASS. 23 TS/SQL parity probes plus idempotent reference replay PASS; H5P revision replay unchanged; existing learning-loop verifier PASS. Formal K012 timeline `1/13` survives both back/next and back/return-current at 390px without a formal review write. Final tests/Student/visual and Remote verdicts remain pending below.

### Local Student Acceptance Mode — PASS

Single Agent explicitly switched from implementation to student acceptance. Fresh anonymous browser completed all 18 Steps of RT01/K012/K021 using real `/api/micro` content and grading (no route mocks): demonstration, exploration, transferred practice, Summary, completed-path review and replay. Formal timelines survived one/multiple back operations and explicit return; all review operations issued zero completion writes. The reusable browser regression is `scripts/acceptance/micro-review.browser.mjs`; latest full run is `/tmp/eduflow-micro-v2-baseline/student-final.log`. An additional 12 rapid back/next pairs issued zero Micro POSTs. Refresh/navigation away returns to persisted progress, discarding transient review.

Existing local student account: CTX01 five pointer category drops, deliberately wrong role, correction, ordering wrong-first feedback and keyboard Space/arrow/drop success PASS. H5P K001: 0/6 wrong classification → Retry → six actual keyboard drops → 6/6 and real completion event; result resubmission, next/back and fresh replay PASS. Completed-path review mounts exactly one H5P player; the hidden formal panel is retained only for a still-active learning position. Local ordinary account differs from the supplied Hosted account, which does not exist in Local Auth; no account was copied or reset.

Read-only `local-before.json` and `local-after.json` compare exactly equal for all of this local student's Path/Unit Progress, Knowledge Evidence and Knowledge States, including original completion timestamps. Review did not duplicate, delete or overwrite evidence, progress or learner state. These private evidence snapshots remain outside the repository.

Visual checks: actual 1440px and 390px captures for Flow, mathematical Simulation and Data Transformation; all have zero horizontal page overflow. Inline/block math, superscripts and numeric update chain were inspected. Fixed the observed translucent header/footer overlap on scroll, misleading click instructions in read-only matrix demonstration and faint disabled matrix values. Footer remains operable above the Assistant. Final browser rerun had zero Micro console errors; an isolated `/api/workflows` 500 occurred during the local login/home transition earlier, outside the Micro routes; no unrelated Workflow changes were made.

Final Local checks: typecheck/lint PASS; 80 files / 499 tests PASS; production build PASS (9.88s), main gzip approximately 1,464.1kB versus approximately 1,384.0kB baseline (KaTeX cost about 80kB gzip plus bundled font assets); existing bundle/H5P vendor CSS warnings retained. Frozen pnpm install PASS. Existing learning-loop verifier PASS. 23 TS/SQL parity probes and content replay PASS, H5P metadata replay unchanged. Fresh Vercel generated build PASS; exactly 12 Functions; native Node generated Micro verifier PASS. Diff review/whitespace PASS. Local acceptance: **PASS**. Remote acceptance: **NOT RUN**, pending this source commit/push and the exact Preview verification. Issue #31 remains OPEN pending that final verification; no issue has been closed or externally commented on.

### Remote Student Acceptance Mode — PASS

Runtime commit `dc9b8d7ac44444ba3f240a2db37629c855cf24db`, actual READY deployment `dpl_5BsMX9FPET1ZKvuammmMrcYjiitg`, https://edu-flow-12h6n5dqm-july-nanas-projects.vercel.app. Vercel's actual build output contains 12 lambda Functions (not inferred from rewrites). Hosted Supabase project `uyljtdbvlivxniililay` applied only `20260905162819_micro_v2_teaching_content.sql`; no schema DDL, table addition, reset or learner-record mutation. All 25 official Reference Steps match Local exactly: content hash `41ae90a3a63b88c4618bd5573b216bd1`.

Authenticated ordinary-account smoke: Courses/Knowledge/Micro/Progress/Workflows 200; the teacher-only submission list (`GET /api/learning` without an assignment locator) correctly returns 403. H5P descriptor/assets loaded through the existing endpoint. A fresh anonymous browser reran the complete 18-Step RT01/K012/K021 new-learner flow and all back-navigation assertions against real Preview APIs; every path completed and review writes remained zero.

The supplied ordinary Hosted account replayed all 25 Steps across RT01/CTX01/K012/K021: missing-edge Flow failure/correction, real calculated Simulation, wrong matrix cell and new-sentence construction, five actual pointer classification drops, keyboard ordering, completed Summary review and refresh all PASS. The run recorded zero completion writes and zero console errors. K001 H5P wrong/unanswered → Retry → six keyboard placements → 6/6 produced the real completion event; repeated result, next/back and fresh replay PASS with exactly one mounted player. Desktop 1440 and narrow 390 screenshots were visually inspected; equations, mechanics and navigation are legible without page overflow.
...
### Content commit, Hosted rollout and exact Preview

Content/documentation commit `8c5cf48b49348af5e78f426953a1918646791e88` was pushed to `feat/gold-course-learning-loop` after Local acceptance. No prototype merge. Supabase CLI dry-run named only `20260906161347_agent_lesson1_teaching_content.sql`; the actual linked push applied exactly that migration, with no seeds or roles. This incrementally updates the existing published Course in project `uyljtdbvlivxniililay`.

Complete Hosted snapshots `hosted-before-rollout.json` and `hosted-after-rollout.json` under `/tmp/eduflow-lesson1` have identical rows and timestamps for all four history tables: 16 Path Progress, 19 Unit Progress, 16 Evidence and 44 Knowledge States. Student learning starts only after this comparison. Teacher's separate read-only Hosted content audit finds zero differences from final source across 10 Paths/10 Units/52 Steps; all ten Knowledge rows and all 117 Course Coverages exactly match the original baseline. Thirty-six source relations remain valid and non-duplicated, including the 33 declared mappings; no dangling Unit/Step/source exists.

Exact content Preview: https://edu-flow-ppw2r22ug-july-nanas-projects.vercel.app — READY, matching commit `8c5cf48b49348af5e78f426953a1918646791e88` and feature branch. The deployment inspection itself reports 12 lambda outputs, matching fresh local generated output; rewrites are not used as Function-count evidence. Root 200; supplied ordinary account login 200; authenticated Health/Courses/Knowledge/Micro/Progress all 200. Actual Preview `/api/micro` matches every authored field in all 52 Steps, and anonymous progress remains empty. Its bundled public Supabase URL matches the linked project. Remote Student continuous learning is in progress; no Remote PASS is claimed yet.

All 42 Local/Hosted migration versions match after rollout. Author independently followed the actual remote Course graph → selected Knowledge → Material detail route for the three corrected primary locators: Harness opened PDF27/307, Agentic Workflow PDF31/307, Sandbox PDF121/307. Each rendered the real textbook page and matching Knowledge context; screenshots and the route log remain in `/tmp/eduflow-lesson1`. This supplements the Student's own source-access exercise and the Teacher's relational audit.

### Remote learning history and inherited mastery policy

After Student completed all ten Paths through normal UI, read-only snapshots show ten new Path Progress, ten new Unit Progress and ten new `micro_path_completed` Evidence rows. All original 16 Path Progress, 19 Unit Progress and 16 Evidence rows are retained byte-for-byte. This normal learning phase is distinct from the earlier content rollout's unchanged four-table snapshots.

Nine Lesson 1 Knowledge states are `learned`; R10 changes from its prior `practicing` to `mastered`. Investigation found an R10 `assignment_accepted` Evidence already present in the pre-rollout snapshot. A separate read-only query confirms its sole required Course Assignment `book-v1-node-r10` was accepted before this Path started. The unchanged server policy in `20260905043000_atomic_learning_transitions.sql` requires nonempty required Learn Paths all completed AND nonempty required Assignments all accepted, and runs on required Path completion. Teacher independently verified this evidence and policy interpretation. R10 therefore satisfies the existing conjunction with this account's old history; it is not Micro-alone mastery or a new Assignment/competence evaluation performed in this round. H02 remains `learned` with its normal completion timestamp update. No user record was reset or downgraded to make statuses look uniform.

Teacher independently graded the Remote post-learning nine answers: **9 correct / 0 partial / 0 incorrect; Remote Closed-book PASS**. Answers again explain the role/fact boundary, two-round feedback, control source, prior approval versus later verification, task-based model comparison and necessary permission versus isolation. This was the same Student role repeating the lesson remotely, not a new human participant or a fresh independent novice sample.

### Remote Student evidence closeout — 2026-09-08

The original Remote Student completed all 52 screens through ordinary Course UI on the exact content Preview, ending at 2026-09-06 17:12:41 UTC (2026-09-07 01:12:41 Asia/Shanghai). Its retained evidence contains all 52 screen texts, 104 desktop/mobile screenshots and layout observations, and the browser request audit. An independent Student closeout reviewer reread the complete evidence after the original reviewer session ended; this is an evidence closeout, not another fresh novice trial. No horizontal overflow or primary learning button outside the viewport was found. The original audit contains 52 complete-step and ten start requests, no failed requests and no console entries. Two Material starts and normal Material progress saves are also present; the complete audit must not be described as a zero-write review window. One next-Path start was recorded under the previous review phase label because the label changed late.

Per-Micro first-screen evidence to last completion intervals are A02 69.6 s; AGC01 57.6 s; R10 94.6 s; AGC02 71.4 s; AGC03 49.0 s; H02 38.3 s; WF05 51.1 s; S01 86.8 s; S02 90.0 s; S03 52.7 s. These are repeated-learning agent/tool observation windows, exclude some navigation and are not validated human learning durations. Authored estimated minutes remain estimates.

The 390 px A02/R10 evidence shows readable compact diagrams; R10's missing-return exercise stops at 4/10 and restores to 10/10. Why review and A02's completed 6/6 state are visible. A fresh 390 px Course-to-Material entry opens actual PDF15/307. Existing non-blocking Runtime observations remain explicit: the floating Assistant partly covers the Material CTA, whose exposed corner still works; resizing an already open desktop Material view to 390 changed page 15 to 30, although fresh mobile entry opens page 15 correctly. These observations were not fixed by content changes. Other retained follow-ups are generic explanation feedback, Assignment-first completion navigation, prerequisite messaging versus graph Quick Learn, graph close-button occlusion and Local React Flow size warnings. None is evidence of a new content/schema/runtime defect introduced by this publication; their causes remain outside this bounded content review.

On resume, the exact content Preview again returned root/login and authenticated Health/Courses/Knowledge/Micro/Progress 200. Every authored field across ten Paths/52 Steps still matches the final source; anonymous Path/Unit progress remains empty. The deployment's queried last-hour 5xx log was empty. Private evidence remains under `/tmp/eduflow-lesson1`, outside Git and without credentials in committed documentation.

The independent closeout reviewer then opened a new ordinary-user browser session for a separate settled-page review check: 2026-09-08 06:52:29.033–06:52:59.912 UTC, completed A02 → Review → Step 3 Why → Back → Next → Return to current progress. The 390 px result remains 6/6 Completed. Its isolated 30.879-second listener recorded one mechanism JavaScript GET, zero non-read requests, zero Micro start/complete requests, zero console/page errors and zero HTTP 5xx. Root independently compared complete Hosted snapshots immediately surrounding this session: Path Progress 28/28, Unit Progress 30/30, Evidence 27/27 and Knowledge States 54/54; every row and timestamp is identical. These current counts are not the earlier rollout baseline. Evidence: `student-remote/closeout-isolated-review.json`, `student-remote/closeout-report.md` and `resume-{before,after}-review.json` in the private evidence directory.

**Final instructional verdict: Teacher Domain Correctness PASS / Instructional Sequence PASS / Material Grounding PASS; Student Local PASS / Remote PASS; post-learning closed-book 9/9 correct for both runs.** Remote PASS is bounded to this Lesson's ordinary-user content and learning flow, not whole-product acceptance, a human efficacy study or independent engineering mastery. The final closeout commit changes only this ledger; its matching deployment still requires READY, real Function-count and authenticated smoke verification before final delivery. Branch remains `feat/gold-course-learning-loop`; no prototype merge.

## Micro instructional continuity / completion context — 2026-09-09

Baseline: branch `feat/gold-course-learning-loop`, local and GitHub HEAD `625b82741aa9d238a02782c29d6638f47c947145`, clean working tree, ahead 8 / behind 0 versus fetched `origin/prototype`. SSH fetch failed once; authenticated HTTPS fetch subsequently succeeded. Baseline typecheck/lint/build and 80 files / 499 tests passed. Existing large-chunk build warning remains.

Hosted project `uyljtdbvlivxniililay` had all 42 repository migrations applied. Fresh queries confirmed the ten Lesson 1 identities/order: A02, AGC01, R10, AGC02, AGC03, H02, WF05, S01, S02, S03. Seven external prerequisite facts exist, including A01 → A02. No explicit Course entry-requirement model was found. Old Graph projection excluded external endpoints while Navigation gated them; both policies required mastered, and Navigation prioritized failed/pending Assignment after Micro. Completion also sent a Micro next action to the return target and Material Reader always returned to Course.

Decision: shared teaching satisfaction is learned/practicing/mastered; explore/learning do not satisfy. Graph, Path and Navigation gate only Course-local factual prerequisites. In-progress Knowledge with unmet prerequisites is blocked consistently across consumers; already learned/practicing state is not downgraded. Required incomplete Micro remains a teaching action; optional Micro/Practice does not hold a learned node in the main route. Mastery policy, Course progress and target fallback are unchanged. New decisions use `course-rule-v2`; v1 history is retained. The small completion resolver uses hydrated real coverage, exact Material Segments and Navigation Knowledge identity. Reader preserves `returnTo` and original Micro source across Segment replacement. Retry responses are discarded on context change.

Content: canonical JSON has exactly ten Summary suffix deletions; all other fields are byte-equivalent after JSON normalization. Removed only manual Course/graph/node/Quick Learn/Material-opening directions. Teaching summaries, source citations, transitions, interactions, identities and order remain. Generator `--content-only --navigation-only` produces `20260908171720_micro_completion_navigation_prose_cleanup.sql`, updating existing Summary bodies with ownership/order checks and no DDL.

Local ordinary-user browser: actual first-time traversal A02 → AGC01 → R10 → AGC02 → AGC03 → H02 → WF05 → S01 → S02 → S03 completed through the shared Micro UI and nine direct Completion Next clicks. No learner reset, direct completion shortcut or Course Graph detour was used. Ten persisted Paths and ten Knowledge states are completed/learned; no Micro-alone mastery. R10 missing feedback stops at 4/10 and reconnecting completes 10/10. A02 Material opened actual PDF15/307, switching page then Reader Back returned to the same completed Micro. Desktop 1440 and mobile 390 completion captures show distinct primary Next and secondary resources, wrapping long titles and no horizontal overflow. S01 classification used mobile controls. Refresh/reopen preserves completion.

An isolated S03 Review → Why → Back/Next → return-to-progress window observed zero non-GET requests. Review plus reload preserved complete database fingerprints: 25 Path Progress, 26 Unit Progress, 25 Evidence, 39 Knowledge rows. No runtime console error occurred in the learning session; the log includes React DevTools notices and a Vite restart during development. Initial local login failed because the supplied test identity did not exist locally; it was created as an ordinary local account without modifying Hosted credentials or existing users/history.

Existing `scripts/acceptance/micro-review.browser.mjs` passed in a fresh anonymous browser context for RT01 and CDS525 K012/K021, including mechanism operation, backward review, complete and no-write review. This is reference regression, not replacement for Lesson 1 acceptance. CDS content/structure was not edited.

Automated verification: `pnpm typecheck`, `pnpm lint`, `pnpm test` (81 files / 512 tests), `pnpm build`, `pnpm exec vercel build`, `pnpm verify:micro:generated`, and `node --env-file=.env.local --import tsx scripts/verify-micro-v2.ts` pass. The latter covers 23 TS/SQL probes, four-reference replay and H5P replay. Actual generated Vercel output contains 12 Functions. Lesson 1 generator and explicit old/new field comparison pass. Build retains the baseline large-chunk warning.

Hosted rollout: dry-run named only `20260908171720`; incremental push applied it without seeds, roles or schema changes. Complete before/after fingerprints remain identical: 29 Path Progress, 31 Unit Progress, 28 Evidence and 55 Knowledge rows. These are rollout-time counts, not the older Lesson 1 publication baseline.

Outstanding acceptance before final verdict: exact new Preview readiness/authenticated smoke/browser checks. Both Local and Hosted have only `golden-micro-AG01` for standalone Learn; it is excluded as a formal acceptance object by the current objective. A clarification about using it solely as standalone technical regression is pending. All published Micro in the two permitted formal Courses have Material coverage, so the no-Material case is covered by resolver regressions but has no existing formal browser sample. These limits are not a claimed browser PASS. No deferred #17/#21/#22/#23/#32 redesign was performed, and no prototype merge is authorized.

First implementation commit `ac59e680c877ce3cdc81e045fcaa838635815683` was pushed without merging prototype. Its exact Preview `https://edu-flow-aikbz1x9t-july-nanas-projects.vercel.app` reached READY with 12 actual lambda outputs. Supplied ordinary-user login and Health/Courses/Knowledge/Micro/Progress/Navigation returned 200; Navigation uses v2, and all 52 Step bodies match the cleaned canonical JSON. Existing Hosted learner history selects CTX02 with a Material teaching entry. This exposed a completion-resolver omission for non-Micro teaching actions: the follow-up reuses exact Segment resolution for a Material next action, rather than hiding that valid next Knowledge. Added a focused regression; final Preview revalidation remains pending.


### Runtime Preview and browser closeout

Runtime commit `beeba6a3297f4a246bce3c021c04a5dd022a9d46` is deployed at `https://edu-flow-pe4qovn67-july-nanas-projects.vercel.app`: READY, actual deployment output 12 lambda Functions. Supplied ordinary-user authentication and Health/Courses/Knowledge/Micro/Progress/Navigation all return 200. All 43 Local/Hosted migration versions match; all 52 Preview Step bodies match canonical JSON. Final typecheck/lint, 81 files / 512 tests and production build pass. No dependency, schema DDL, Function or CDS teaching change was added.

A separate ordinary Hosted QA identity was created to verify first-time learning without resetting the supplied learner's history. It completed all 52 Lesson 1 Steps through real UI, with nine direct Next clicks and no Course Graph detour. Read-only validation finds ten completed Paths, ten learned Knowledge states and ten unique `micro_path_completed` Evidence sources. None is mastered. R10's missing return stops execution at 4/10; restoring it permits 10/10. Each completed Micro was captured at 1440 and 390 pixels, with no horizontal overflow; long Material titles wrap and Next retains primary emphasis. The first runner timed out because it checked the previous completion DOM immediately after the route changed; waiting for the next lesson article resolved this harness race, with no product edit or learner reset. A02 was completed in that first segment; the remaining nine were completed after resuming AGC01.

On the same runtime Preview, the supplied ordinary learner opened A02 related Material at PDF15/307, moved to PDF16, and returned to the same A02 completion. Its next teaching action correctly opened Chat Template Material at exact PDF53/307. This preserves distinct behavior for supplemental Material and a Material-based next Knowledge. The fresh learner's S03 completion correctly offers CTX01, so S03 is not misrepresented as the end of the whole Course.

An isolated Hosted S03 Review → Why → Next → Back → return-to-progress → reload observed zero application writes and zero runtime errors. One external `vercel.live/login/validate` POST came from the Preview toolbar; the initial all-hosts request assertion flagged it, so the result is deliberately reported as zero application writes rather than zero network POSTs. Full before/after fingerprints are identical: 39 Path Progress, 41 Unit Progress, 38 Evidence and 65 Knowledge rows. This confirms no duplicate formal completion, progress rollback or learner-state downgrade. Evidence and screenshots are private temporary files outside Git.

Issue #21 records the shared prerequisite/external scope/v2 decisions; #22 retains Attempt/PerformanceResult and stronger competency evidence. #31 was closed only after rereading the deterministic mechanism/definition feedback boundary, verifying its existing reason/mapping tests and repeating Agent/CDS/Review regressions. #32 remains OPEN and now records the actual remaining browser-sample gaps.

Bounded verdict: **BLOCKED on formal acceptance evidence**, not an observed Lesson 1 continuity failure. Standalone, Course-without-Material and whole-route-no-next behavior have automated regressions but no permitted existing formal browser samples. Global Golden is excluded by the objective; no temporary Course, fabricated assets, deleted coverage or reset learner state was used to substitute for those cases. The pending clarification does not count as approval. The implemented/tested work is committed and pushed, but the full success criteria do not yet support declaring “stop platform work and enter Lesson 2.” No deferred #17/#21/#22/#23/#32 redesign was performed. This closeout changes documentation only; the matching final Git Preview must still pass readiness/Function-count/authenticated smoke before delivery.
