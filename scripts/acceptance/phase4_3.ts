...
import { compareMappingCounts, evaluateCourseMapping, type MappingGold } from "../../src/features/course/mapping/evaluation";
...
async function execute() {
  const prepared = await repository.prepare({ courseId, ownerId, provider: env.llmProvider, model: env.llmModel, promptVersion: COURSE_MAPPING_PROMPT_VERSION, schemaVersions: ["course-assignments-v1", "assignment-dependencies-v1"] });
  try {
    const generated = await runCourseMappingPipeline({ runtime: prepared.runtime, knowledgeNodes: prepared.knowledgeNodes, knowledgeEdges: prepared.knowledgeEdges, workflowTemplates: prepared.workflowTemplates }, createJsonGenerationClient(env));
...
const replayPrepared = await repository.prepare({ courseId, ownerId, provider: env.llmProvider, model: env.llmModel, promptVersion: COURSE_MAPPING_PROMPT_VERSION, schemaVersions: ["course-assignments-v1", "assignment-dependencies-v1"] });
await repository.persist(replayPrepared.runId, first.plan);
const second = { prepared: replayPrepared, plan: first.plan };
...
const practiceMatch = await semanticMatches(practicesGold.practices.map((practice) => ({ id: practice.id, text: [practice.title, practice.objective, practice.deliverable].join("。") })), runtime.assignments.map((assignment) => ({ id: assignment.id, text: [assignment.title, assignment.description, assignment.expectedOutput].join("。") })), 0.65);
const outcomeMatch = await semanticMatches(outcomesGold.chapterOutcomes.map((outcome) => ({ id: outcome.id, text: outcome.title })), runtime.chapterOutcomes.map((outcome) => ({ id: outcome.id, text: [outcome.title, ...runtime.assignmentOutcomeCompositions.filter((composition) => composition.outcomeId === outcome.id).flatMap((composition) => { const assignment = runtime.assignments.find((item) => item.id === composition.assignmentId); return assignment ? [assignment.title, assignment.description, assignment.projectContribution ?? ""] : []; })].join("。") })), 0.55);
...
const report = { datasetVersion: "mapping-gold-v0.1", sourceKnowledgeRunId: sourceRunId, courseId, runs: [first.prepared.runId, second.prepared.runId], matching: { knowledge: knowledgeMatch.decisions, practices: practiceMatch.decisions, outcomes: outcomeMatch.decisions }, evaluation, idempotency: compareMappingCounts(before, after), product: { knowledgeNavigation: navigation, segmentProjectionChecks, skillTree: { knowledgeNodeCount: skillTree.knowledgeNodes.length, assignmentCount: skillTree.assignmentSummary.assignmentCount } } };
