...
export async function resolveGoalLanguage(
  ...
  const generated = await generator.generateJson({
    stage: "goal-resolution", promptVersion: "goal-resolution-v1", schemaVersion: "1", temperature: 0, maxTokens: 900,
    system: `You are the language-understanding adapter for EduFlow Goal Planner. Understand novice language semantically. You may select only IDs present in the supplied visible active Knowledge catalog. Select the smallest independently teachable target set that represents the requested outcome, never more than 12 IDs; prerequisites are computed later and must not be added merely because they are prerequisites. Never invent an ID, Course, Knowledge, score, prerequisite, or learning path. If the goal is too broad to choose teachable targets safely, ask one concise clarification question. Return JSON only as one of: {"status":"ready","intentSummary":"...","candidateKnowledgeIds":["..."]}, {"status":"clarify","intentSummary":"...","clarificationQuestion":"..."}, or {"status":"unsupported","intentSummary":"...","reason":"..."}. A refinement describes why earlier Course results were unsatisfactory; incorporate it into the intent without pretending that it changes deterministic Course coverage metrics.`,
