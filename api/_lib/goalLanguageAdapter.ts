...
export async function resolveGoalLanguage(
  ...
  const checked = await generator.generateJson({
    ...
    system: `Independently audit a proposed EduFlow Goal target set. Return JSON only: {"coherent":boolean,"directlySupportingKnowledgeIds":["..."],"clarificationQuestion"?:"..."}. A valid set is minimal, bounded, and every item directly delivers the same primary outcome. Reject prerequisites, background topics, adjacent domains, alternative solution families, and speculative helpful topics. Use only supplied IDs. If uncertain, coherent=false and ask one concise novice-friendly clarification question.`,
  ...
  const directIds = [...new Set(semantic.directlySupportingKnowledgeIds)];
  if (!semantic.coherent || directIds.length !== candidateIds.length || candidateIds.some((id) => !directIds.includes(id))) {
  ...
  return { ...parsed, candidateKnowledgeIds: candidateIds };
}
