...
export default handleApi(async (request: VercelRequest, response: VercelResponse) => {
  ...
  if (request.method === "GET") {
    const [knowledgeResult, coursesResult, assignmentsResult, materialsResult] = await Promise.all([
    ...
    const knowledge = dataOrThrow(knowledgeResult.data as Row[] | null, knowledgeResult.error, "UserKnowledgeState query").map((row) => ({
      ...
      sourceNodeIds: row.source_node_ids ?? undefined, evidence: row.evidence ?? undefined, updatedAt: String(row.updated_at)
    }));
