...
export default handleApi(async (request: VercelRequest, response: VercelResponse) => {
  ...
  const prepared = await repository.prepare({ courseId, ownerId: user.id, provider: env.llmProvider, model: env.llmModel, promptVersion: COURSE_MAPPING_PROMPT_VERSION, schemaVersions: ["course-assignments-v1", "assignment-dependencies-v1"] });
  try {
    const generation = await runCourseMappingPipeline({ runtime: prepared.runtime, knowledgeNodes: prepared.knowledgeNodes, knowledgeEdges: prepared.knowledgeEdges, workflowTemplates: prepared.workflowTemplates }, createJsonGenerationClient(env));
    const plan = buildCourseMappingPlan(prepared.runtime, generation);
    await repository.persist(prepared.runId, plan);
    json(response, 201, { run: { id: prepared.runId, status: "completed", promptVersion: COURSE_MAPPING_PROMPT_VERSION }, result: { courseId, materialCoverageCount: plan.materialKnowledgeCoverages.length, assignmentCount: plan.assignments.length, assignmentCoverageCount: plan.assignmentCoverages.length, dependencyCount: plan.assignmentDependencies.length, outcomeCount: plan.chapterOutcomes.length, finalProjectCount: plan.finalProjects.length } });
  } catch (error) {
