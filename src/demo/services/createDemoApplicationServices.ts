...
export function createDemoApplicationServices(): ApplicationServices {
  ...
  return {
    ...
    learnerStateService: { startMaterial: async () => ({ status:"learning" }), startAssignment: async () => ({ status:"started" }), submitAssignment: async () => ({ status:"submitted", accepted:false }) } as never,
