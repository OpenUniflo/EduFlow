...
const simulation: SimulationDefinition = { type: "simulation", mode: "explore", parameter: { label: "学习率", min: .02, max: 1.4, step: .02, initial: .1 }, model: { kind: "quadratic-descent", curvature: 2, optimum: 0, initial: 4, steps: 12 }, target: { maxLoss: .02, maxGrowth: 1 } };
const transformation: TransformationDefinition = { type: "data-transform", mode: "explore", corpus: [["I", "love", "AI"], ["I", "love", "deep", "learning"], ["I", "enjoy", "learning"]], vocabulary: ["I", "love", "AI", "deep", "learning", "enjoy"], window: 1 };
const step = (id: string, kind: MicroStep["kind"], title: string, body: string, interaction?: MicroInteraction, successFeedback?: string, retryFeedback?: string): MicroStep => ({ id, kind, title, body, interaction, successFeedback, retryFeedback });
...
export const microV2References: MicroLearningPath[] = [
  path("aiad-rt01-agent-loop", "RT01", "ai-agents-in-depth", "Agent Loop：从调用意图到可靠回复", 10, [
    ...
    step("aiad-rt01-explain", "explanation", "先分清模型与框架", "模型返回 tool_calls 表示‘请调用这个工具’，不等于工具已执行。框架执行后，把 assistant 的调用消息和带 tool_call_id 的 tool 结果加入历史，再调用模型。只有模型不再返回 tool_calls 时，本例才结束。下面使用一个工具和示意天气数据。"),
    ...
    step("aiad-rt01-summary", "summary", "你现在能解释 Agent Loop", "你能区分：模型产生调用意图，框架执行工具，tool 消息回传结果，模型再决定，最后无 tool_calls 时结束。换成搜索、库存或数据库查询时，仍应检查这条信息链是否完整。来源：课程原书第 2 章，PDF 第 40–47 页。"),
  ]),
  path("aiad-ctx01-message-context", "CTX01", "ai-agents-in-depth", "Message Context：谁说了什么，模型看到了什么", 10, [
    ...
    step("aiad-ctx01-explain", "explanation", "角色表示来源，tools 描述能力", "system 是行为约束；user 是用户请求；assistant 是模型的文字或 tool_calls；tool 是框架执行后返回的数据。tools 是独立字段，描述可调用工具及参数，并非聊天消息。下一次调用需要携带历史：模型不会自动知道框架刚刚做了什么。"),
  ...
  path("cds525-k012-learning-rate", "CDS525-K012", "cds525-deep-learning", "Learning Rate：看见更新如何收敛", 10, [
    ...
    step("cds525-k012-explain", "explanation", "每一步都由同一个公式计算", "θ下一步 = θ − η × ∇C(θ)。η 是学习率，梯度指出 Loss 上升方向，所以我们减去它。用简单的 C(θ)=θ²：梯度是 2θ，最小值 θ=0。θ=4、η=0.1 时，梯度 8，更新 −0.8，下一步 θ=3.2，Loss 从 16 降到 10.24。"),
    ...
    step("cds525-k012-insight", "feedback", "决定稳定性的还有曲率", "这个模型里，误差每一步乘上 (1 − η×曲率)。绝对值小于 1 时误差衰减；负号表示来回跨过最小值；绝对值大于 1 时误差放大。曲率变大，同样的 η 会带来更大的更新，因此没有所有任务通用的学习率区间。"),
    step("cds525-k012-challenge", "application", "换一个更陡的 Loss", "现在 C(θ)=2(θ−1)²，曲率从 2 变成 4，最小值移到 θ=1。当前 η=0.8 会怎样？先观察，再调整，让 12 次更新后的 Loss 达标且不超过初始 Loss。", { ...simulation, mode: "challenge", parameter: { ...simulation.parameter, initial: .8 }, model: { ...simulation.model, curvature: 4, optimum: 1, initial: 5 } }, "你根据曲率变化调整了步长，并用计算后的 Loss 验证结果。", "曲率更大时，相同学习率会产生更大更新；根据轨迹与误差衰减调整。"),
    step("cds525-k012-summary", "summary", "你能用轨迹判断学习率", "学习率控制沿负梯度方向移动多少。太小会慢；合适时误差缩小；太大可能摆动甚至发散。换 Loss 时要重新观察曲率、轨迹和 Loss，不能照搬固定区间。本例是可解释的二次函数，不代表所有深度模型的完整行为。来源：Lecture 2，第 50 页的参数更新公式。"),
  ]),
  path("cds525-k021-cooccurrence-matrix", "CDS525-K021", "cds525-deep-learning", "Co-occurrence：词的邻居如何变成向量", 12, [
    ...
    step("cds525-k021-explain", "explanation", "一次事件只增加一个 cell", "固定窗口为左、右各 1 个词，不跨句。句子 I love AI 中，以 love 为中心，会遇到左边 I、右边 AI：分别给 love 行的 I 列、AI 列加 1。换 I 为中心时，再给 I 行的 love 列加 1。行是中心词，列是上下文词。"),
    ...
    step("cds525-k021-summary", "summary", "你知道这些数字从哪里来", "共现矩阵 = 文本 + 明确窗口规则 + 逐事件计数。行是中心词，列是上下文词，一行给出上下文向量。换语料或窗口要重新计算事件，不能沿用原矩阵。来源：Lecture 3，第 25–26 页；探索使用第 26 页原始三句语料，练习是更小的迁移例句。"),
  ]),
];
