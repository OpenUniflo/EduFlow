...
const unavailable: LessonAssistantResult = {fallback:true,message:"当前课件没有可用的 Demo Assistant 能力。"};
function isGoldenArticle(material: { courseId:string; type:string }) {
  return material.courseId === GOLDEN_COURSE_ID && material.type === "article";
}
...
export const demoLessonAssistantProvider: LessonAssistantProvider = {
  listActions(material){return isGoldenArticle(material) ? Object.entries(scripts).map(([id,script])=>({id,label:script.label})) : [];},
  resolveAction(material,actionId){if(!isGoldenArticle(material)) return unavailable;const script=scripts[actionId];return script?result(actionId,script):fallback;},
  resolveText(material,input){if(!isGoldenArticle(material)) return unavailable;const normalized=input.trim().toLowerCase();const match=["alignment","timeout","example","simplify"].map((id)=>[id,scripts[id]] as const).find(([,script])=>script.keywords.some((keyword)=>normalized.includes(keyword)));return match?result(match[0],match[1]):fallback;}
};
