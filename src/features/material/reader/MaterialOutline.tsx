...
export function MaterialOutline({ material, activeSegmentId, collapsed, onToggle, onSelect }: {
  ...
  return <aside className="atlas-lesson-outline glass-v2">
    ...
    {!collapsed ? <>
      ...
      <div className="atlas-outline-list">{sortMaterialSegments(material).map((segment) => <button key={segment.id} className={segment.id === activeSegmentId ? "active" : ""} aria-current={segment.id === activeSegmentId ? "true" : undefined} onClick={() => onSelect(segment.id)}><span>{String(segment.page ?? segment.order).padStart(2, "0")}</span><div><strong>{segment.title ?? `第 ${segment.page ?? segment.order} 页`}</strong><small>{segment.section ?? material.type.toUpperCase()}</small></div></button>)}</div>
    </> : null}
  </aside>;
}
