...
export function MaterialRenderer({ material, activeSegmentId, zoom, navigationRequest, onVisibleSegmentChange, onNavigationSettled }: {
  ...
  if (material.type === "pdf" && material.source?.kind === "pdf") {
    ...
    return <PdfMaterialViewer courseId={material.courseId} materialId={material.id} sourcePageCount={material.source.pageCount} segments={segments} activePage={activeSegment?.page ?? activeSegment?.order ?? 1} zoom={zoom} navigationRequest={navigationRequest} onVisiblePageChange={(page) => {
