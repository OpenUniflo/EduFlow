...
function CourseSearchCard({ message, content, context }: { message: AssistantMessage; content: CourseSearchTimelineContent; context: AssistantContextSnapshot }) {
  ...
  return <section className="assistant-goal-plan assistant-timeline-card" aria-label="课程搜索结果">
    ...
    {ready ? <>
      ...
      {!content.plan.matches.length ? <p>当前没有可访问的课程候选，但仍可整理一份个性化 Course Creation Brief。</p> : null}
    ...
    </> : <div className="assistant-goal-unresolved"><strong>{content.plan.resolution.status === "ambiguous" ? "目标存在歧义" : "当前 Knowledge 不支持该目标"}</strong><p>{content.plan.resolution.reason}</p></div>}
  </section>;
}
...
export function AssistantConversation({ context, compact = true }: { context: AssistantContext; compact?: boolean }) {
  ...
  async function submit() { const value = input.trim(); if (!value || runtime.sending) return; setInput(""); if (goalMode) await runtime.planGoal(value, contextSnapshot); else await runtime.send(value, contextSnapshot); }
