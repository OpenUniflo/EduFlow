...
export function CourseCreationWorkspacePage({ session, onLogout, resolver }: { session: MockSession; onLogout(): void; resolver: CourseCreationScenarioResolver }) {
  ...
  useEffect(() => {
    ...
    void apiRequest<{ course: CourseRuntimeData | null; courseId: string | null; lifecycle: "draft" | "published" | null }>(`/api/courses?creationBriefMessageId=${encodeURIComponent(briefMessageId)}`).then((result) => {
      if (!alive) return;
      if (!result.course || !result.courseId || !result.lifecycle) { setRecoveryChecked(true); return; }
      ...
      if (!alive) return;
      setBriefError(error instanceof Error ? error.message : "课程草稿恢复失败");
      setRecoveryChecked(true);
    });
    return () => { alive = false; };
  }, [briefMessageId]);
  ...
  const runtimeForAssetCheck = persistedRuntime ?? previewRuntime;
  const executableMicroKnowledgeIds = runtimeForAssetCheck ? includedIds.filter((nodeId) => Boolean(applicationServices.microLearningRepository.getLesson(nodeId, { courseId: runtimeForAssetCheck.course.id }))) : [];
  ...
  const assetAudit = persistedRuntime
    ? auditCourseAssetCoverage(persistedRuntime, { microKnowledgeIds: executableMicroKnowledgeIds, practiceEmphasis: Boolean(design?.assets.desiredAssignmentKnowledgeIds.length) })
    : previewRuntime
      ? auditCourseAssetCoverage(previewRuntime, { microKnowledgeIds: executableMicroKnowledgeIds, practiceEmphasis: Boolean(design?.assets.desiredAssignmentKnowledgeIds.length) })
      : null;
...
function AssetsResult({ design, includedIds, executableMicroKnowledgeIds, titleById, files, sourceRuntime, setProposal }: any) {
  const a = design.assets;
  const availableMicroKnowledgeIds = [...new Set([...a.availableMicroKnowledgeIds, ...executableMicroKnowledgeIds])];
  ...
  return <><ResultTitle icon={<FileText/>} step="STEP 4 RESULT" title="学习内容准备情况"/><div className="creator-asset-stats"><span>当前学习资料<strong>{a.materialKnowledgeIds.length} / {includedIds.length}</strong><small>可复用 {a.availableMaterialKnowledgeIds.length}</small></span><span>当前快速学习<strong>{a.microKnowledgeIds.length} / {includedIds.length}</strong><small>可用 {availableMicroKnowledgeIds.length}</small></span><span>当前实践任务<strong>{a.assignmentKnowledgeIds.length} / {includedIds.length}</strong><small>可复用 {a.availableAssignmentKnowledgeIds.length}</small></span></div><div className="creator-coverage-table"><div className="head"><b>学习内容</b><b>学习资料</b><b>快速学习</b><b>实践任务</b></div>{includedIds.map((id: string) => <div key={id}><strong>{titleById.get(id) ?? id}</strong>{(["material", "micro", "assignment"] as const).map((type) => { const current = a[`${type}KnowledgeIds`].includes(id); const available = type === "micro" ? availableMicroKnowledgeIds.includes(id) : a[`available${type[0].toUpperCase()}${type.slice(1)}KnowledgeIds`].includes(id); const planned = desired(id, type); return <button type="button" key={type} onClick={() => toggle(id, type)} title="调整希望安排的学习内容">{current ? "已包含" : available ? "可用" : planned ? "希望安排" : "缺少"}</button>; })}</div>)}</div><div className="creator-warning"><strong>缺少学习内容不会阻止课程草稿创建</strong><p>{includedIds.length - a.materialKnowledgeIds.length} 项缺学习资料 · {includedIds.length - new Set([...a.microKnowledgeIds, ...availableMicroKnowledgeIds]).size} 项缺快速学习 · {includedIds.length - a.assignmentKnowledgeIds.length} 项缺实践任务</p>{files.length ? <p><Upload size={14}/>已选择本次参考资料；当前版本不会自动解析其内容，也不会伪造覆盖关系。</p> : null}{sourceRuntime ? <p>参考课程现有 {sourceRuntime.materials.length} 份学习资料、{sourceRuntime.assignments.length} 个实践任务；上表只把真实映射显示为“可复用”，不会默认复制。</p> : null}</div></>;
}
function DraftResult({ design, includedCount, validation, graphData, runtime, assetAudit }: any) {
  ...
  return <><ResultTitle icon={<FileText/>} step="STEP 5 RESULT" title={runtime ? "已保存的课程草稿" : "课程草稿设计预览"}/><div className="creator-draft-hero"><span className="creator-lifecycle draft">{runtime ? "已保存草稿" : "草稿预览"}</span><h2>{runtime?.course.title ?? design.requirements.goal}</h2><p>{chapterCount} 个篇章 · {knowledgeCount} 个学习内容 · {runtime?.materials.length ?? 0} 份学习资料 · {runtime?.assignments.length ?? 0} 个实践任务</p><div className="creator-validation"><span className={validation.valid ? "pass" : "fatal"}>课程结构 <strong>{validation.valid ? "正常" : "需要修正"}</strong></span><span>学习内容准备 <strong>{assetAudit?.actionability.noExecutableActivities ? "当前不可执行" : assetAudit?.issues.some((item: any) => item.severity === "warning") ? "有缺口提醒" : "正常"}</strong></span></div></div>{assetAudit?.actionability.noExecutableActivities ? <div className="creator-warning"><strong>课程结构已经创建，但当前没有可执行学习活动</strong><p>快速学习、学习资料和实践任务目前均为 0；可以继续保存草稿或完成结构创建，但不能把它视为已经可以开始学习。</p>{assetAudit.practice.assignmentCoverageGap ? <p><strong>实践覆盖不足：</strong>当前目标强调实践，但还没有真实实践任务。</p> : null}</div> : assetAudit?.practice.assignmentCoverageGap ? <div className="creator-warning"><strong>实践覆盖仍有缺口</strong><p>{assetAudit.assignments.missingKnowledgeCount} 个学习内容尚无真实实践任务。</p></div> : null}<div className="creator-mini-graph"><CourseGraph graphData={graphData} view="full" focusedChapterId={null} mode="knowledge" selectedId={null} searchMatchId={null} {...graphCallbacks}/></div></>;
}
function PublishResult({ runtime, graphData, assetAudit, validation }: any) {
  return <><ResultTitle icon={<Rocket/>} step="STEP 6 RESULT" title="最终预览与完成检查"/><div className="creator-learner-preview"><header><span className="atlas-kicker">课程预览</span><h2>{runtime.course.title}</h2><p>{runtime.course.description}</p></header><div className="creator-mini-graph"><CourseGraph graphData={graphData} view="full" focusedChapterId={null} mode="knowledge" selectedId={null} searchMatchId={null} {...graphCallbacks}/></div></div><div className="creator-publish-check"><h3>服务端课程检查</h3>{validation?.valid ? <><p className="pass"><Check size={15}/>课程结构正常</p><p className="pass"><Check size={15}/>{validation.knowledgeCount} 个学习内容身份与课程关系有效</p><p className="pass"><Check size={15}/>没有失效的课程关系</p></> : <p>正在读取服务端检查结果…</p>}{assetAudit?.actionability.noExecutableActivities ? <p className="warn">课程结构已经创建，但当前没有可执行学习活动。</p> : null}{assetAudit?.practice.assignmentCoverageGap ? <p className="warn">实践覆盖不足：目标强调实践，但实践任务仍有缺口。</p> : null}{assetAudit?.issues.filter((issue: any) => issue.severity === "warning").map((issue: any) => <p className="warn" key={issue.code}>提醒 · {issue.message}</p>)}<small>学习资料、快速学习或实践任务缺失只会提醒，不阻止完成创建。</small></div></>;
}
