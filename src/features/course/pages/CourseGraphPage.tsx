...
export function CourseGraphPage({ session, onLogout, courseDesignAssistantProvider, microLearningProvider }: { session: MockSession | null; onLogout: () => void; courseDesignAssistantProvider?:CourseDesignAssistantProvider; microLearningProvider?:MicroLearningProvider }) {
  ...
  const overviewReturnTarget = authoringRoute ? "/teaching" : "/courses";
  ...
  return (
    <main className="atlas-graph-page" data-experience={experience}>
      ...
      <header className="atlas-skill-course-island">
        <div className="atlas-skill-header-left"><button onClick={() => view === "overview" ? navigate(overviewReturnTarget) : changeView("overview")} aria-label={authoringRoute ? "返回教学管理" : "返回课程中心"}><ArrowLeft size={18} /></button><span className="atlas-skill-divider" /><div className="atlas-skill-title"><span>{runtime.course.title}</span><strong>{presentation === "path" ? "课程路线" : view === "overview" ? "课程篇章总览" : view === "focused" ? "聚焦篇章" : `完整课程${mode === "knowledge" ? "技能树" : "实训树"}`}</strong>{view === "focused" && presentation === "graph" ? <small>/ {courseChapters.find((item) => item.id === focusedChapterId)?.title}</small> : null}</div>{personalDraftPreview && runtime.course.creationBriefMessageId ? <button className="atlas-skill-focus" onClick={() => window.location.assign(`/courses/create?briefId=${encodeURIComponent(runtime.course.creationBriefMessageId!)}`)}>返回课程创建</button> : null}</div>
