...
export function CourseGraphPage({ session, onLogout, courseDesignAssistantProvider, microLearningProvider }: { session: MockSession; onLogout: () => void; courseDesignAssistantProvider?:CourseDesignAssistantProvider; microLearningProvider?:MicroLearningProvider }) {
  ...
  async function launchMicroLearning(nodeId:string){if(!runtime||!microLearningProvider?.getLesson(nodeId,{courseId:runtime.course.id}))return;await applicationServices.learnerStateService.startKnowledge(nodeId);await refreshLearnerState(session.userId);const target=createMicroLearningNavigation(nodeId,{courseId:runtime.course.id,returnTo:`${location.pathname}${location.search}`});navigate(target.to,{state:target.state});}
  async function launchMaterialLearning(nodeId:string,materialId:string){await applicationServices.learnerStateService.startKnowledge(nodeId);await refreshLearnerState(session.userId);navigate(materialLink(materialId));}
