import { ArrowDown, ArrowLeft, ArrowRight, ArrowUp, BookOpen, Check, Clock3, Crosshair, FileText, Layers3, Maximize2, Minus, Network, Play, Plus, Redo2, Search, Settings2, Sparkles, Target, Trash2, Undo2, WandSparkles, X } from "lucide-react";
...
const knowledgeStatusLabel = { completed: "已掌握", learning: "学习中", available: "可学习", locked: "未解锁" } as const;
...
export function CourseGraphPage({ session, onLogout, courseDesignAssistantProvider, microLearningProvider }: { session: MockSession | null; onLogout: () => void; courseDesignAssistantProvider?:CourseDesignAssistantProvider; microLearningProvider?:MicroLearningProvider }) {
  ...
  async function startSelectedKnowledge(nodeId:string){if(!runtime||personalDraftPreview)return;if(!session){navigate("/login",{state:authGateState(location)});return;}await applicationServices.learnerStateService.startKnowledge(nodeId,runtime.course.id);await refreshLearnerState(session.userId);}
  async function launchMicroLearning(nodeId:string){if(!runtime||personalDraftPreview||!microLearningProvider?.getLesson(nodeId,{courseId:runtime.course.id}))return;if(session){await applicationServices.learnerStateService.startKnowledge(nodeId,runtime.course.id);await refreshLearnerState(session.userId);}const target=createMicroLearningNavigation(nodeId,{courseId:runtime.course.id,returnTo:`${location.pathname}${location.search}`});navigate(target.to,{state:target.state});}
  async function launchMaterialLearning(nodeId:string,materialId:string){if(!runtime||personalDraftPreview)return;if(session){await applicationServices.learnerStateService.startKnowledge(nodeId,runtime.course.id);await refreshLearnerState(session.userId);}navigate(materialLink(materialId));}
  ...
  return (
    <main className="atlas-graph-page" data-experience={experience}>
      ...
      <header className="atlas-skill-course-island">
        <div className="atlas-skill-header-left"><button onClick={() => view === "overview" ? navigate("/courses") : changeView("overview")} aria-label="返回上一级"><ArrowLeft size={18} /></button><span className="atlas-skill-divider" /><div className="atlas-skill-title"><span>{runtime.course.title}</span><strong>{presentation === "path" ? "课程路线" : view === "overview" ? "课程篇章总览" : view === "focused" ? "聚焦篇章" : `完整课程${mode === "knowledge" ? "技能树" : "实训树"}`}</strong>{view === "focused" && presentation === "graph" ? <small>/ {courseChapters.find((item) => item.id === focusedChapterId)?.title}</small> : null}</div></div>
      ...
      {personalDraftPreview ? <section className="creator-warning"><strong>课程草稿 / Draft Preview</strong><p>可以浏览课程结构和内容预览；完成创建前不会记录学习进度、开始快速学习或提交实践任务。</p>{runtime.course.creationBriefMessageId ? <button className="atlas-primary" onClick={() => window.location.assign(`/courses/create?briefId=${encodeURIComponent(runtime.course.creationBriefMessageId!)}`)}>返回课程创建</button> : null}</section> : null}
