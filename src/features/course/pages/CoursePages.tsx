...
export function CourseCenterPage({ session, onLogout }: { session: MockSession; onLogout: () => void }) {
  ...
  const recent = [...courses].filter(({ state }) => state.isActive).sort((left, right) => (right.summary.updatedAt ?? "").localeCompare(left.summary.updatedAt ?? ""))[0];
  ...
  return (
    <main className="atlas-page-shell atlas-course-center">
      ...
      <div className="atlas-content-wrap">
        <section className="atlas-course-title atlas-course-title-row"><div><h1>课程中心</h1><p>Published 课程可供学习；我的课程只包含你主动开始的课程。</p></div><span className="atlas-pill">{courses.length} 门公开课程 · {learningCount} 门我的课程</span></section>
        <nav className="learning-view-tabs" aria-label="课程范围"><button className={tab === "mine" ? "active" : ""} onClick={() => setTab("mine")}>我的课程</button><button className={tab === "all" ? "active" : ""} onClick={() => setTab("all")}>全部课程</button></nav>
        ...
        <section className="atlas-course-section">
          ...
          <div className="atlas-course-grid">
            {visible.map(({ runtime, summary, state }) => <article className="atlas-course-card" key={runtime.course.id} onClick={() => navigate(`/courses/${runtime.course.id}`)}><div className="atlas-card-accent" style={{ background: runtime.course.accentColor }} /><div className="atlas-pill">{state.isActive ? "我的课程" : "公开课程"} · {runtime.course.title}</div><h3>{runtime.course.subtitle ?? runtime.course.title}</h3><p>{runtime.course.description}</p>{pathPreview(runtime.chapters)}<div className="atlas-course-meta"><span>{summary.lessonCount} 课</span><span>{summary.knowledgeNodeCount} 原子节点</span><span>{summary.assignmentCount} 实训</span></div><div className="atlas-progress-row"><span>实训进度</span><div className="atlas-progress-track"><i style={{ width: `${summary.progress}%` }} /></div><strong>{summary.progress}%</strong></div><div className="atlas-card-actions">{state.isActive ? <><button className="atlas-primary" onClick={(event) => { event.stopPropagation(); navigate(`/courses/${runtime.course.id}`); }}>继续课程</button><button className="atlas-secondary" disabled={membershipBusy === runtime.course.id} onClick={(event) => { event.stopPropagation(); void setMembership(runtime.course.id, false); }}>移出我的课程</button></> : <button className="atlas-primary" disabled={membershipBusy === runtime.course.id} onClick={(event) => { event.stopPropagation(); void setMembership(runtime.course.id, true); }}>开始课程</button>}</div></article>)}
