  ...
  return <main className="assignment-page">
    ...
    <div className="assignment-page-grid">
      ...
      <article className="assignment-experience-card glass-v2">
        {submitted ? <div className="assignment-submitted"><Check size={34} /><h2>本次提交已记录</h2><p>Assignment 完成状态已更新；Knowledge mastery 仍只由独立证据决定。</p></div> : experience.type === "answer" ? <AnswerExperience prompt={experience.prompt} onSubmit={submit} /> : experience.type === "code" ? <CodeExperience experience={experience} onSubmit={submit} /> : experience.type === "trace" ? <TraceExperience experience={experience} onSubmit={submit} /> : <section className="assignment-experience-body"><h2>Workflow Canvas</h2><p>{experience.prompt}</p><div className="assignment-workflow-preview"><Network size={34} /><strong>继承型工作流已准备</strong><span>Canvas 将从现有 Planner、Workers 与 Merge 结构开始。</span></div><button className="atlas-primary" onClick={() => navigate(workflowLaunchUrl({courseId,assignmentId:assignment.id,workflowTemplateId:assignment.workflowTemplateId!}))}>进入 Workflow Canvas <ArrowRight size={15} /></button></section>}
      </article>
    </div>
  </main>;
}
