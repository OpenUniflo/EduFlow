...
export function CourseNavigator({ graph, runtime, knowledge, courseState, authenticated, loadNavigation, onSelect, onAction, onSignIn, microMinutes }: {
  ...
  onSignIn(): void; microMinutes(id: string): number | undefined;
}) {
  ...
  const model = useMemo(() => buildCourseNavigator({ graph, runtime, knowledge, courseState, decision: result.decision }), [graph, runtime, knowledge, courseState, result.decision]);
  const action = model.nextAction;
  const minutes = action?.nodeId && action.resourceKind === 'micro' ? microMinutes(action.nodeId) : undefined;
  ...
  return <div className="course-navigator"><div className="navigator-columns">
    <aside className="navigator-queue" aria-label="课程行动队列">
      <motion.section className="navigator-next" key={action?.resourceId ?? result.error ?? 'empty'} initial={reduced ? false : { opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: .2 }} aria-label="下一步" aria-live="polite">
        <span className="atlas-kicker">下一步</span>
        {!authenticated ? <><h2>登录后继续你的路线</h2><p>为你保留学习进度与实训待办。</p><button className="atlas-primary" onClick={onSignIn}>登录开始学习 <ArrowRight size={16} /></button></> : result.loading ? <p role="status">正在安排下一步…</p> : result.error ? <><p role="alert">{result.error}</p><button className="atlas-secondary" onClick={() => setRetry(value => value + 1)}>重试</button></> : action ? <><h2>{action.title}</h2><small>{action.label}{minutes ? <> · <Clock3 size={12} /> {minutes} 分钟</> : null}</small><p>{action.reason}</p><button className="atlas-primary" onClick={() => onAction(action)}>{action.cta} <ArrowRight size={16} /></button></> : <><h2>{model.complete ? '这段路线，已学完' : '暂时没有可开始的动作'}</h2><p>{model.notice ?? '课程路线仍可浏览，学习活动准备好后会显示在这里。'}</p></>}
      </motion.section>
      <section className="navigator-backlog" aria-label="实训待办"><h2>实训待办 <span>· {authenticated ? model.pendingPractices.length : '—'}</span></h2>
        {!authenticated ? <p>登录后查看你的实训待办。</p> : !model.pendingPractices.length ? <p>暂无实训待办。达到学习条件后，相关任务会出现在这里。</p> : <>
