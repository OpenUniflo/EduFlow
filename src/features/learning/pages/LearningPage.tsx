...
export function LearningPage({session,onLogout,microLearningProvider}:{session:MockSession;onLogout():void;microLearningProvider:MicroLearningProvider}){
  ...
  function launchMicroLearning(){if(!current)return;const target=createMicroLearningNavigation(current.knowledgeId,{courseId:current.courseId,returnTo:"/"});navigate(target.to,{state:target.state});}
