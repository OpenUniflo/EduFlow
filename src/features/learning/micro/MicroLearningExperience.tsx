...
export function MicroLearningExperience({session,onLogout,repository}:{session:MockSession|null;onLogout():void;repository:MicroLearningRepository}) {
  ...
  const returnTarget=resolveMicroLearningReturnTarget(location.state,courseId); const context=session?{workspace:"learning" as const,experienceMode:"learn" as const,userRole:session.role,capabilities:session.capabilities,courseId,knowledgeId}:undefined;
  useEffect(()=>{setAnswer(initialAnswer(step?.interaction));setGrading(null);setWhyOpen(false);},[step?.id]);
  const completeCurrent=useCallback(async(submission?:MicroLearningAnswer|{kind:"h5p-result";contentRef:string;eventId:string;result:H5PResult})=>{if(!path||!unit||!step||busy)return;setBusy(true);setPinned({unitId:unit.id,stepId:step.id});try{const result=await repository.completeStep(path.id, unit.id, step.id,submission??(step.interaction?answer:undefined));setGrading(result.correct?"success":"retry");if(result.correct&&session)await refreshLearnerState(session.userId);else if(!result.correct)setPinned(null);}catch{setGrading("error");setPinned(null);}finally{setBusy(false);}},[answer,busy,path,repository,session,step,unit]);
  ...
  const feedback=grading==="success"?(step.successFeedback??(session?"判断正确，进度已保存。":"判断正确；本次匿名体验不会保存进度。")):grading==="retry"?(step.retryFeedback??"答案尚未满足完成条件，请重试。"):grading==="error"?(session?"保存失败，进度没有被推进。请重试。":"检查失败，请重试；匿名体验没有产生持久记录。"):null;
