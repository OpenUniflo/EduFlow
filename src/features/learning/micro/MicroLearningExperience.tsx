...
export function MicroLearningExperience({session,onLogout,provider}:{session:MockSession;onLogout():void;provider:MicroLearningProvider}){
  ...
  if(!lesson)return <main className="micro-learning-page"><GlobalNav active="learning" session={session} onLogout={onLogout}/><section className="atlas-empty-state"><h1>该知识暂不支持微学习</h1><p>Prototype 只为经过验证的 Golden Knowledge 提供 MicroLesson，不会自动生成不可靠内容。</p><button className="atlas-primary" onClick={()=>navigate(returnTarget)}>返回</button></section></main>;
  ...
  function advance(completedIds=completedStepIds){if(!completedIds.has(step.id))return;if(index<resolvedLesson.steps.length-1){setIndex((value)=>value+1);setAnswer("");setGradingFeedback(null);setAssistantMessage("Assistant 可以解释或提示，但不会替你答题或推进步骤。");return;}if(!canCompleteMicroLesson(resolvedLesson.steps,completedIds))return;recordMicroLearningActivity({userId:session.userId,knowledgeId,lessonId:resolvedLesson.id,courseId,completedAt:new Date().toISOString()});setCompleted(true);}
  ...
  return <main className="micro-learning-page"><GlobalNav active="learning" session={session} onLogout={onLogout}/>
    <header className="micro-learning-header glass-v2"><button onClick={()=>navigate(returnTarget)}><ArrowLeft size={16}/>返回</button><span><small>MICRO LEARNING · {lesson.estimatedMinutes} 分钟</small><strong>{lesson.title}</strong></span><i>{index+1} / {lesson.steps.length}</i></header>
