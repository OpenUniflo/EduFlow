...
describe("Active Today Knowledge",()=>{
  ...
  it("keeps standalone Knowledge and resolves a global Micro independently of Course context",()=>{
    ...
    expect(item.resources.primaryCourse).toBeUndefined();
    expect(item.resources.micro).toMatchObject({available:true,source:"global"});
  });
  it("preserves every Course context for shared Knowledge",()=>{
    ...
    expect(item.resources.courses.map((course)=>course.courseId)).toEqual(["second","course"]);
  });
});
