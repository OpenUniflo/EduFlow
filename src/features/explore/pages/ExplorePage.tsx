...
export function ExplorePage({session,onLogout}:{session:MockSession|null;onLogout():void}){
  const navigate=useNavigate(); const location=useLocation(); const sceneRef=useRef<KnowledgeAtlasSceneHandle>(null); const governance=useDomainGovernance();
  ...
  function requireLogin(){navigate("/login",{state:authGateState(location)});}
  async function startKnowledge(nodeId:string){if(!session||starting)return;setStarting(true);try{await applicationServices.learnerStateService.startKnowledge(nodeId,selectedContext?.kind==="course"?selectedContext.courseId:undefined);await refreshLearnerState(session.userId);}finally{setStarting(false);}}
  async function openMicro(){if(!selected||!selectedContext?.micro.path)return;const courseId=selectedContext.kind==="course"?selectedContext.courseId:undefined;if(session){await applicationServices.learnerStateService.startKnowledge(selected.id,courseId);await refreshLearnerState(session.userId);}const target=createMicroLearningNavigation(selected.id,{courseId,returnTo:"/explore"});navigate(target.to,{state:target.state});}
  async function openMaterial(material:KnowledgeMaterialResource){if(!selected)return;if(session){await applicationServices.learnerStateService.startKnowledge(selected.id,material.courseId);await refreshLearnerState(session.userId);}navigate(buildMaterialDeepLink({courseId:material.courseId,materialId:material.materialId,segmentId:material.segmentId}));}
