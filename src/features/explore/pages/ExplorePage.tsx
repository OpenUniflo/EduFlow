...
export function ExplorePage({session,onLogout}:{session:MockSession;onLogout():void}){
  ...
  const [selectedId,setSelectedId]=useState<string|null>(null); const [contextByKnowledge,setContextByKnowledge]=useState<Record<string,string>>({}); const [searchMatchId,setSearchMatchId]=useState<string|null>(null); const [query,setQuery]=useState(""); const [paused,setPaused]=useState(false); const [starting,setStarting]=useState(false);
  ...
  const assistantContext={workspace:"explore" as const,experienceMode:"learn" as const,userRole:session.role,capabilities:session.capabilities,courseId:selectedContext?.kind==="course"?selectedContext.courseId:undefined,knowledgeId:selectedId??undefined};
  ...
  async function startKnowledge(nodeId:string){if(starting)return;setStarting(true);try{await applicationServices.learnerStateService.startKnowledge(nodeId,selectedContext?.kind==="course"?selectedContext.courseId:undefined);await refreshLearnerState(session.userId);}finally{setStarting(false);}}
  async function openMicro(){if(!selected||!selectedContext?.micro.path)return;const courseId=selectedContext.kind==="course"?selectedContext.courseId:undefined;await applicationServices.learnerStateService.startKnowledge(selected.id,courseId);await refreshLearnerState(session.userId);const target=createMicroLearningNavigation(selected.id,{courseId,returnTo:"/explore"});navigate(target.to,{state:target.state});}
  async function openMaterial(material:KnowledgeMaterialResource){if(!selected)return;await applicationServices.learnerStateService.startKnowledge(selected.id,material.courseId);await refreshLearnerState(session.userId);navigate(buildMaterialDeepLink({courseId:material.courseId,materialId:material.materialId,segmentId:material.segmentId}));}
  async function openAssignment(assignment:KnowledgeAssignmentResource){await applicationServices.learnerStateService.startAssignment(assignment.courseId,assignment.assignmentId);await refreshLearnerState(session.userId);navigate(`/courses/${assignment.courseId}/assignments/${assignment.assignmentId}`);}
  return <main className="explore-page"><GlobalNav active="explore" session={session} onLogout={onLogout}/>
    ...
    <EduFlowAssistant context={assistantContext} contextLabel={selected?.title??"Global Knowledge Atlas"} className="explore-assistant"/>
  </main>;
}
