...
export function DomainManagementPage({ session, onLogout }: { session: MockSession; onLogout: () => void }) {
  ...
  function createNewDomain() {
    ...
    const created = createDomain({ name: newName.trim(), description: "管理员创建的知识领域。", scope: "global" });
    ...
    <main className="domain-admin-page">
      ...
      {tab === "management" ? <section className="domain-admin-grid">
        ...
        <aside className="domain-detail-panel domain-panel">{selectedDomain ? <>
          <div className="domain-detail-hero"><i style={{ background: selectedDomain.canonicalColor }} /><span><small>{selectedDomain.scope.toUpperCase()} DOMAIN</small><h2>{selectedDomain.name}</h2><p>{counts.get(selectedDomain.id) ?? 0} nodes</p></span></div>
          <label><span>Name</span><input value={selectedDomain.name} onChange={(event) => updateDomain(selectedDomain.id, { name: event.target.value })} /></label>
          <label><span>Description</span><textarea value={selectedDomain.description ?? ""} onChange={(event) => updateDomain(selectedDomain.id, { description: event.target.value })} /></label>
          <label><span>Canonical Color</span><div className="domain-color-input"><Palette size={16} /><input key={selectedDomain.canonicalColor} defaultValue={selectedDomain.canonicalColor} onBlur={(event) => { if (/^#[0-9a-f]{6}$/i.test(event.target.value)) updateDomain(selectedDomain.id, { canonicalColor: event.target.value }); else event.target.value = selectedDomain.canonicalColor; }} /></div></label>
          <div className="domain-color-palette">{DOMAIN_COLOR_PALETTE.map((color) => <button key={color} style={{ background: color }} className={selectedDomain.canonicalColor.toLowerCase() === color.toLowerCase() ? "active" : ""} onClick={() => updateDomain(selectedDomain.id, { canonicalColor: color })} aria-label={`使用颜色 ${color}`} />)}</div>
          <div className="domain-scope-row"><span>Scope</span><strong>{selectedDomain.scope === "global" ? "Global" : "Tenant"}</strong></div>
          <button className="domain-archive" onClick={() => { updateDomain(selectedDomain.id, { status: "archived" }); setSelectedDomainId(""); }}><Archive size={15} />Archive Domain</button>
