-- Keep semantic utterances, deterministic UI actions, and creator recovery state
-- structurally distinct. Recreate the draft RPC with qualified columns so its
-- TABLE output parameters cannot shadow persisted column names.

alter table public.assistant_messages
  add column message_kind text not null default 'utterance'
  check (message_kind in ('utterance', 'action', 'goal_clarification'));

alter table public.courses
  add column creator_metadata jsonb
  check (creator_metadata is null or jsonb_typeof(creator_metadata) = 'object');

drop function public.create_personal_course_draft_for_brief(uuid,uuid,text,text,text[],jsonb);

create function public.create_personal_course_draft_for_brief(
...
begin
  if p_owner_user_id is null or p_creation_brief_message_id is null
    or length(btrim(coalesce(p_goal_text, ''))) = 0 or length(p_goal_text) > 1000
    or p_creator_metadata is null or jsonb_typeof(p_creator_metadata) <> 'object' then
  ...
  if coalesce(cardinality(all_knowledge_ids), 0) = 0
    or (select count(distinct valueset.value) from unnest(all_knowledge_ids) valueset(value)) <> cardinality(all_knowledge_ids)
    or (select count(distinct valueset.value) from unnest(p_target_knowledge_ids) valueset(value)) <> cardinality(p_target_knowledge_ids) then
    raise exception 'invalid_personal_course_draft_knowledge' using errcode = '22023';
  end if;
  if exists (select 1 from unnest(p_target_knowledge_ids) valueset(value) where not (valueset.value = any(all_knowledge_ids))) then
  ...
  if replacing_existing then
    delete from public.course_target_knowledge target where target.course_id = created_course_id;
    delete from public.curriculum_sequences sequence_row where sequence_row.course_id = created_course_id;
    delete from public.curriculum_coverages coverage where coverage.course_id = created_course_id;
    delete from public.curriculum_lessons lesson where lesson.course_id = created_course_id;
    delete from public.curriculum_chapters chapter_row where chapter_row.course_id = created_course_id;
    update public.courses course_row
    set title = left(btrim(p_goal_text),80), target_outcome = btrim(p_goal_text), source_course_id = p_source_course_id,
      creator_metadata = p_creator_metadata,
      revision = 'creator-draft-' || extract(epoch from clock_timestamp())::bigint::text, updated_at = now()
    where course_row.id = created_course_id and course_row.lifecycle = 'draft' and course_row.owner_user_id = p_owner_user_id;
  else
    insert into public.courses(id,title,description,target_outcome,accent_color,generation_status,lifecycle,course_type,owner_user_id,source_course_id,author_user_id,revision,creation_brief_message_id,creator_metadata)
    values(created_course_id,left(btrim(p_goal_text),80),'根据课程创建需求与已确认学习范围创建的个人课程。',btrim(p_goal_text),'#7567e8','draft','draft','personal',p_owner_user_id,p_source_course_id,p_owner_user_id,'creator-draft-1',p_creation_brief_message_id,p_creator_metadata);
...
revoke all on function public.create_personal_course_draft_for_brief(uuid,uuid,text,text,text[],jsonb,jsonb) from public, anon, authenticated;
grant execute on function public.create_personal_course_draft_for_brief(uuid,uuid,text,text,text[],jsonb,jsonb) to service_role;
