-- Course.target_outcome is optional metadata. PostgreSQL cannot remove one
-- statement from a stored function, so the effective base publish function is
-- restated in full with only the historical target-outcome guard removed.
